// node_modules/typelessform-widget/dist/zh-D8YTaiRw.js
var e = {
  ...
  "error.no_fields_detected": "无法识别填写表单的信息。请尝试更清楚地指定应在字段中输入哪些数据。",
  ...
  "consent.warning": "请勿说出密码、支付详情、政府标识符或医疗信息。这些数据不适用于语音功能处理。",
  ...
  "consent.summary_transfer": "OpenAI(美国)用于AI处理 • Google Cloud/Firebase(欧盟、波兰)用于托管和安全",
  "consent.summary_card3_title": "您的权利",
  "consent.summary_rights": "随时撤回同意 • 请求访问或删除您的数据 • <a href='https://webappski.com/en/legal/product-privacy' target='_blank' rel='noopener noreferrer'>隐私政策</a>中的完整详细信息",
  "consent.full_details_title": "阅读完整详细信息",
  "consent.section1_title": "我们处理哪些数据?",
  "consent.data1": "语音录音。音频文件通过我们的服务器发送到OpenAI Whisper进行转录。处理后文件不会保存在服务器上。",
  "consent.data2": "语音转录(文本)。发送到OpenAI GPT用于表单填写。默认情况下,我们不在系统中保存转录;在技术日志中我们仅存储元数据(文本长度、语言、时长)。重要提示:OpenAI可能会存储请求数据长达30天以防止滥用;OpenAI不提供提前删除;数据不用于模型训练。",
  "consent.data3": "表单字段元数据。名称/类型/占位符(排除敏感字段)。",
  "consent.data4": "页面URL。由我们的基础设施处理(用于兼容性/诊断),不发送到OpenAI。请避免在URL查询参数中包含个人数据。",
  "consent.data5": "浏览器User-Agent。在本地和我们的基础设施中使用(兼容性/安全),不发送到OpenAI。",
  "consent.data6": "IP地址。由Google Cloud/Firebase基础设施记录用于安全(最多30天),不发送到OpenAI。",
  "consent.section2_title": "数据传输到何处以及基于何种法律依据?",
  "consent.recipients": "接收方。OpenAI(美国)——音频/文本处理;Google Cloud/Firebase(欧盟/波兰)——托管和安全日志。数据不用于模型训练。",
  "consent.transfer1": "OpenAI(美国)。向OpenAI发送的请求(包括音频/文本)可能会被OpenAI存储长达30天以防止滥用;数据不用于模型训练。传输到美国——基于欧盟委员会批准的SCC。",
  "consent.basis_title": "法律依据:",
  "consent.basis1": "语音功能——您的同意(GDPR第6(1)(a)条)。",
  "consent.basis2": "技术日志(IP/UA/URL)——合法利益(第6(1)(f)条)——安全和调试。",
  "consent.section3_title": "您的权利",
  "consent.rights1": "您可以随时在小部件设置中撤回同意;这不影响撤回前处理的合法性。",
  "consent.rights2": "除撤回同意外,您还有权通过网站所有者请求访问或删除存储在我们系统中的数据(技术日志、成本指标)。<a href='https://webappski.com/en/legal/product-privacy' target='_blank' rel='noopener noreferrer'>隐私政策</a>中的完整详细信息。",
  "consent.rights3": "此功能适用于16岁以上用户;对于年轻用户,需要父母/监护人同意。",
  ...
  "consent.footer_sccs": "向美国的传输根据SCC进行。详细信息可在我们的<a href='https://webappski.com/en/legal/product-privacy' target='_blank' rel='noopener noreferrer'>隐私政策</a>中查看。",
  "consent.accept": "我接受",
  // Consent checkboxes
...
//# sourceMappingURL=zh-D8YTaiRw-Y4KRBPNX.js.map
